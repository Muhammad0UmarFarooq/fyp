    ...
    <div class="flex">
        ...
        <main class="flex-1 p-4 md:p-8">
            <div class="max-w-8xl mx-auto bg-[#161e2d] rounded-3xl overflow-hidden shadow-2xl border border-white/5">
                ...
                <!-- Action Buttons -->
                <div class="p-4 md:p-6 flex flex-col sm:flex-row flex-wrap justify-start sm:justify-end gap-3 md:gap-4">
                    @if($pitch)
                        <form action="{{ route('entrepreneur.pitch.destroy', $pitch) }}" method="POST" class="inline w-full sm:w-auto">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full sm:w-auto bg-[#ff0000] text-white px-8 py-3 rounded-lg font-bold text-base sm:text-lg hover:bg-red-700 transition-colors" onclick="return confirm('Are you sure you want to delete this pitch?');">
                                Delete Pitch
                            </button>
                        </form>
                        <a href="{{ route('entrepreneur.pitch.update', $pitch) }}" class="w-full sm:w-auto bg-[#4ade80] text-[#064e3b] px-8 py-3 rounded-lg font-bold text-base sm:text-lg hover:bg-[#3dbd6d] transition-colors inline-block text-center">
                            Update Pitch
                        </a>
                    @else
                ...
                @if($pitch)
                <!-- Pitch Content -->
                <div class="px-4 md:px-8 pb-8 md:pb-12">
                    <!-- Video/Image Preview -->
                    <div class="relative w-full h-[40vh] md:h-[70vh] bg-gray-900 rounded-2xl overflow-hidden mb-8 group">
                        @if($pitch->video_path)
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
                        <div class="min-w-0">
                            <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold mb-2 break-words">{{ $pitch->startup_name }}</h1>
                        ...
                        <div class="text-left md:text-right">
                            ...
                            <div class="text-2xl sm:text-3xl md:text-4xl font-bold text-brand-green break-words">Rs {{ number_format($pitch->funding_required ?? 0) }}</div>
                        </div>
                    </div>
                    ...
                    <!-- Stats Cards Grid -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8">
                        ...
                        <div class="bg-[#0b1120] p-4 sm:p-6 rounded-xl border border-white/5">
                            ...
                            <div class="text-lg sm:text-2xl font-bold text-white break-words">Rs {{ number_format($pitch->monthly_net_value) }}</div>
                        </div>
                        @endif
                        ...
                        <div class="bg-[#0b1120] p-4 sm:p-6 rounded-xl border border-white/5">
                            ...
                            <div class="text-lg sm:text-2xl font-bold text-brand-green">{{ $pitch->monthly_growth }}%</div>
                        </div>
                        @endif
                        
                        @if($pitch->return_time)
                        <div class="bg-[#0b1120] p-4 sm:p-6 rounded-xl border border-white/5">
                            <div class="text-[10px] font-bold text-gray-500 tracking-[0.2em] uppercase mb-2">Return Time (years)</div>
                            <div class="text-lg sm:text-2xl font-bold text-brand-green">{{ $pitch->return_time }}</div>
                        </div>
                        @endif
                        ...
                        @if($pitch->pitchFields)
                            ...
                            <div class="bg-[#0b1120] p-4 sm:p-6 rounded-xl border border-white/5">
                                ...
                                <div class="text-lg sm:text-2xl font-bold text-white break-words">{{ $field->value }}</div>
                            </div>
                            @endforeach
                        @endif
                    </div>
                ...
                @else
                <div class="px-4 md:px-8 pb-8 md:pb-12 flex flex-col items-center justify-center text-center py-20">
                    <div class="bg-brand-green/10 p-6 rounded-full mb-6">
                        <svg class="w-12 h-12 sm:w-16 sm:h-16 text-brand-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                        </svg>
                    </div>
                    <h2 class="text-2xl sm:text-3xl font-bold text-white mb-4">No Active Pitch</h2>
